// Strategy: for each iteration, it finds the least dense point and sets up an antenna at the densest point containing it.
...
int get_less_dense(int N, double r, Point *points) {
    int min_cov = numeric_limits<int>::max( );
    int best_point = -1;
...
int set_next(int N, double r, Point *points) {
    int less_dense_point = get_less_dense(N, r, points);

    return get_denser(N, r, points, less_dense_point);
}
